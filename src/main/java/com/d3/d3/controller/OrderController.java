...
import com.d3.d3.service.OrderNotFoundException;
...
public class OrderController {
    ...
    public String deleteOrder(@PathVariable String id, Model m, HttpSession session) {
        String redir = Functions.goAdmin(session);
        if (redir.isEmpty()) {
    ...
    public String showOrderAdmin(@PathVariable String id, Model m, HttpSession session) {
        String redir = Functions.goAdmin(session);
        if (redir.isEmpty()) {
            int id_ = Functions.getInt(id);
            if (id_ <= 0) {
    ...
    public String changeStatusOrder_get(@PathVariable String id, Model m, HttpSession session) {
        String redir = Functions.goAdmin(session);
        if (redir.isEmpty()) {
            int id_ = Functions.getInt(id);
            if (id_ <= 0) {
    ...
    public String changeStatusOrder_post(@ModelAttribute(value = "statuserror") @Valid StatusOrder so,
        ...
        if (redir.isEmpty()) {
            ...
            try {
                update = orderService.updateStatus(so.getIdOrd(), so.getStatus());
    ...
    public String showOrderUser(@PathVariable String id,
        ...
        if (redir.isEmpty()) {
            ...
            if (idUser <= 0) {
                m.addAttribute("error", "El pedido no existe"); //Aquí engañamos al usuario :P
            } else {
